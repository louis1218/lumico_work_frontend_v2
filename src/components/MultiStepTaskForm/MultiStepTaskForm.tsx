...
export default function MultiStepTaskForm({
  ...
  const [realignmentPhotos, setRealignmentPhotos] = useState<{
    before: File[];
    after: File[];
    general: File[];
  }>({ before: [], after: [], general: [] });
  const [realignmentPreviews, setRealignmentPreviews] = useState<{
    before: string[];
    after: string[];
    general: string[];
  }>({ before: [], after: [], general: [] });
  const [realignmentMeta, setRealignmentMeta] = useState<RealignmentMeta>({
    uploadedBeforePreviews: [],
    uploadedAfterPreviews: [],
    uploadedGeneralPreviews: [],
    uploadedBeforeCount: 0,
    uploadedAfterCount: 0,
    uploadedGeneralCount: 0,
    isSaved: false,
  });
    ...
    try {
      ...
      if (Array.isArray(data.tasks)) {
        const mergedTasks: TaskState[] = form.tasks
          ...
          .map((task) => {
            ...
            return {
              ...
              uploadedBeforePreviews: (m?.before_photos || []).map(
                (k: string) => `${S3_BASE_URL}/${k}`,
              ),
              uploadedAfterPreviews: (m?.after_photos || []).map(
                (k: string) => `${S3_BASE_URL}/${k}`,
              ),
        ...
        const otherTasks: TaskState[] =
          otherFromDB.length > 0
            ? otherFromDB.map((t: any, i: number) => ({
                name: `Other (${i + 1})`,
                customName: t.task_name,
                ...
                uploadedBeforePreviews: (t.before_photos || []).map(
                  (k: string) => `${S3_BASE_URL}/${k}`,
                ),
                uploadedAfterPreviews: (t.after_photos || []).map(
                  (k: string) => `${S3_BASE_URL}/${k}`,
                ),
                isSubmitted: !!data.submitted,
                isSaved: !data.submitted && !!t.created_by,
                notes: t.notes || "",
                beforePhotos: [],
                beforePreviews: [],
                afterPhotos: [],
      ...
      const realignment = data.tasks?.find(
        (t: any) => t.task_name === "Realignment Check",
      );
      if (realignment) {
        setRealignmentMeta({
          uploadedBeforePreviews: (realignment.before_photos || [])
            .filter((k: string) => k.includes("realignment_before"))
            .map((k: string) => `${S3_BASE_URL}/${k}`),
          uploadedAfterPreviews: (realignment.after_photos || [])
            .filter((k: string) => k.includes("realignment_after"))
            .map((k: string) => `${S3_BASE_URL}/${k}`),
          uploadedGeneralPreviews: (realignment.before_photos || [])
            .filter((k: string) => k.includes("realignment_general"))
            .map((k: string) => `${S3_BASE_URL}/${k}`),
          uploadedBeforeCount: (realignment.before_photos || []).filter(
            (k: string) => k.includes("realignment_before"),
          ).length,
          uploadedAfterCount: (realignment.after_photos || []).filter(
            (k: string) => k.includes("realignment_after"),
          ).length,
          uploadedGeneralCount: (realignment.before_photos || []).filter(
            (k: string) => k.includes("realignment_general"),
          ).length,
        ...
        setForm((prev) => ({
          ...prev,
          need_realignment_fix: data.need_realignment_fix ?? false,
        }));
      }
    } catch (err) {
  ...
  const uploadGroup = async (
    ...
    const sanitizedTask = taskName
      ? taskName.replace(/\s+/g, "_").toLowerCase()
      : "realignment";
  ...
  const handleNext = async () => {
    if (stepIndex === 0) {
      if (!form.block || !form.trackerId) {
        alert("Please fill in Block and Tracker ID.");
        return;
      }
  ...
  const handleBack = async () => {
    ...
    const isEmptyOther =
      lastTask?.name?.startsWith("Other") &&
      !lastTask.isSaved &&
      !lastTask.isSubmitted &&
      !lastTask.uploadedBeforeCount &&
      !lastTask.uploadedAfterCount &&
      !lastTask.notes?.trim();
  ...
  const handleSave = async () => {
    if (!form.trackerId || !form.block)
      return alert("Tracker ID and Block are required.");
    ...
    if (task.needsRepair === null || task.needsRepair === undefined)
      return alert("Please select whether the task needs repair.");
    if (task.name.startsWith("Other") && !task.customName?.trim())
      return alert("Please enter a task name for this 'Other' task.");
    ...
    try {
      ...
      const existingBefore =
        task.uploadedBeforePreviews?.map((u) =>
          u.replace(`${S3_BASE_URL}/`, ""),
        ) || [];
      const existingAfter =
        task.uploadedAfterPreviews?.map((u) =>
          u.replace(`${S3_BASE_URL}/`, ""),
        ) || [];
      ...
      const taskPayload = {
        ...
        needsRepair: task.needsRepair,
        isComplete: task.isComplete,
        before_photos: [
          ...existingBefore,
          ...uploaded.filter((i) => i.type === "before").map((i) => i.key),
        ],
        after_photos: [
          ...existingAfter,
          ...uploaded.filter((i) => i.type === "after").map((i) => i.key),
        ],
  ...
  const handleSaveRealignment = async () => {
    ...
    if (needsRepair === null)
      return alert("Please select whether repair is needed.");
    if (
      needsRepair === true &&
      realignmentPhotos.before.length === 0 &&
      realignmentMeta.uploadedBeforePreviews.length === 0
    )
    ...
    try {
      ...
      const uploaded = needsRepair
        ...
        : await uploadGroup(
            realignmentPhotos.general,
            "general",
            form.trackerId,
          );

      const taskPayload = {
        task_name: "Realignment Check",
        remark: "realignment_check",
        checked: realignmentChecked,
        needs_repair: needsRepair,
        before_photos: needsRepair
          ? [
              ...realignmentMeta.uploadedBeforePreviews.map((u) =>
                u.replace(`${S3_BASE_URL}/`, ""),
              ),
              ...uploaded.filter((i) => i.type === "before").map((i) => i.key),
            ]
          : [
              ...realignmentMeta.uploadedGeneralPreviews.map((u) =>
                u.replace(`${S3_BASE_URL}/`, ""),
              ),
              ...uploaded.filter((i) => i.type === "general").map((i) => i.key),
            ],
        after_photos: [],
        created_by: session.username,
        updated_at: new Date().toISOString(),
        notes:
          form.realignmentNotes?.trim() ||
          (needsRepair === false
            ? "No repair needed"
            : "Realignment request photos"),
      };
  ...
  const handleSubmit = async () => {
    const unsaved = form.tasks.filter(
      (t) => t.name !== "Other" && !t.isSaved && !t.isSubmitted,
    );
    if (unsaved.length > 0) {
      alert(
        `Please save all tasks:\n\n${unsaved.map((t) => `• ${t.name}`).join("\n")}`,
      );
      return;
    }
    if (!realignmentMeta.isSaved && !realignmentMeta.isSubmitted) {
      alert("Please save the Realignment Check.");
      return;
    }
    ...
    try {
      ...
      const payload = {
        ...
        tasks: [
          ...
          ...form.tasks.map((task) => ({
            task_name: task.name.startsWith("Other")
    ...
    <div className={styles.page}>
      ...
      <div className={styles.formWrap}>
        ...
        {step !== "init" && (
          <div className={styles.formTopBar}>
            ...
            {step !== "final" && (
              <button
                type="button"
                className={styles.ghostBtnGreen}
                onClick={() => setStepIndex(form.tasks.length + 2)}
              >
                Go to Summary →
              </button>
            )}
          ...
          <div className={styles.stepNav}>
            ...
            <span className={styles.stepLabel}>
              {step === "realignmentCheck"
                ? "Realignment Check"
                : `Task ${stepIndex - 1} of ${form.tasks.length}`}
            </span>
            <button
              type="button"
              className={styles.navArrow}
        ...
        <form
          ...
          onSubmit={(e) => {
            e.preventDefault();
            step === "final" ? handleSubmit() : handleNext();
          }}
        >
          {/* ── INIT ── */}
          {step === "init" && (
            <div className={styles.initStep}>
              ...
              <div className={styles.field}>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setForm((f) => ({ ...f, trackerId: e.target.value }))
                  }
                  placeholder="e.g. T12"
                  required
                />
            ...
            <div className={styles.stepBody}>
              <label className={styles.checkRow}>
                <input
                  ...
                  checked={realignmentChecked}
                  onChange={(e) => {
                    setRealignmentChecked(e.target.checked);
                    setNeedsRepair(null);
                  }}
                />
                <span>Checked</span>
              ...
              {realignmentChecked && needsRepair !== null && (
                <>
                  ...
                  <input
                    ...
                    ref={(ref) => {
                      photoInputRef.current = {
                        ...photoInputRef.current,
                        [needsRepair ? "realignBefore" : "realign"]: ref,
                      };
                    }}
                    onChange={(e) => {
                      const files = Array.from(e.target.files || []);
                      const key = needsRepair ? "before" : "general";
                      setRealignmentPhotos((p) => ({
                        ...p,
                        [key]: [...p[key], ...files],
                      }));
                      setRealignmentPreviews((p) => ({
                        ...p,
                        [key]: [
                          ...p[key],
                  ...
                  <div
                    ...
                    onClick={() =>
                      needsRepair
                        ? photoInputRef.current.realignBefore?.click()
                        : photoInputRef.current.realign?.click()
                    }
                  >
                    +
                  </div>
                  <div className={styles.previewGrid}>
                    {[
                      ...(needsRepair
                        ? realignmentMeta.uploadedBeforePreviews
                        : realignmentMeta.uploadedGeneralPreviews),
                      ...(needsRepair
                        ? realignmentPreviews.before
                        : realignmentPreviews.general),
                    ].map((src, i) => {
                      const isSaved = needsRepair
                        ? realignmentMeta.uploadedBeforePreviews.includes(src)
                        : realignmentMeta.uploadedGeneralPreviews.includes(src);
                      return (
                          ...
                          {isSaved ? (
                            ...
                            <button
                              ...
                              onClick={(e) => {
                                ...
                                setRealignmentPreviews((p) => ({
                                  ...p,
                                  [key]: p[key].filter((s) => s !== src),
                                }));
                                setRealignmentPhotos((p) => ({
                                  ...p,
                                  [key]: p[key].filter(
                                    (_, idx) =>
                                      URL.createObjectURL(p[key][idx]) !== src,
                                  ),
                                }));
                              }}
                            >
                  ...
                  <div className={styles.field}>
                    ...
                    <textarea
                      ...
                      rows={3}
                      placeholder="Add any notes for this realignment check"
                      value={form.realignmentNotes || ""}
                      onChange={(e) =>
                        setForm((prev) => ({
                          ...prev,
                          realignmentNotes: e.target.value,
                        }))
                      }
                    />
                  </div>
            ...
            <div className={styles.stepBody}>
                ...
                {form.tasks[stepIndex - 2].name.startsWith("Other") ? (
                  <>
                    <input
                      ...
                      value={form.tasks[stepIndex - 2].customName || ""}
                      placeholder="Other Task Name"
                      onChange={(e) => {
                        const updated = [...form.tasks];
                        updated[stepIndex - 2].customName = e.target.value;
                        setForm((f) => ({ ...f, tasks: updated }));
                      }}
                    />
                    {form.tasks
                      .filter((t) => t.name.startsWith("Other"))
                      .indexOf(form.tasks[stepIndex - 2]) > 0 && (
                      <button
                        type="button"
                        className={styles.deleteTaskBtn}
                        onClick={() => {
                          const updated = [...form.tasks];
                          updated.splice(stepIndex - 2, 1);
                          setForm((f) => ({ ...f, tasks: updated }));
                          setStepIndex((prev) => Math.max(prev - 1, 1));
                        }}
                      >
                  ...
                  <h3 className={styles.taskTitle}>
                    {form.tasks[stepIndex - 2].name}
                  </h3>
                )}
                {stepIndex === form.tasks.length + 1 &&
                  form.tasks[form.tasks.length - 1]?.name.startsWith(
                    "Other",
                  ) && (
                    <button
                      type="button"
                      className={styles.addMoreBtn}
                      onClick={addNewOtherTask}
                    >
                      + More
                    </button>
                  )}
              ...
              <label className={styles.checkRow}>
                <input
                  ...
                  onChange={(e) => {
                    const updated = [...form.tasks];
                    updated[stepIndex - 2].checked = e.target.checked;
                    updated[stepIndex - 2].needsRepair = null;
                    setForm((f) => ({ ...f, tasks: updated }));
                  }}
                />
                <span>Checked</span>
                  ...
                  <div className={styles.radioRow}>
                    <label className={styles.radioLabel}>
                      <input
                        type="radio"
                        name="needsRepair"
                        checked={currentTask.needsRepair === false}
                        onChange={() => {
                          const u = [...form.tasks];
                          u[stepIndex - 2].needsRepair = false;
                          setForm((f) => ({ ...f, tasks: u }));
                        }}
                      />{" "}
                      No
                    ...
                    <label className={styles.radioLabel}>
                      <input
                        type="radio"
                        name="needsRepair"
                        checked={currentTask.needsRepair === true}
                        onChange={() => {
                          const u = [...form.tasks];
                          u[stepIndex - 2].needsRepair = true;
                          setForm((f) => ({ ...f, tasks: u }));
                        }}
                      />{" "}
                      Yes
              ...
              {currentTask.checked && currentTask.needsRepair === true && (
                <>
                    ...
                    <div className={styles.checkboxRow}>
                      <label className={styles.checkRow}>
                        <input
                          type="checkbox"
                          checked={
                            form.tasks[stepIndex - 2].isComplete === true
                          }
                          onChange={() => {
                            const u = [...form.tasks];
                            u[stepIndex - 2].isComplete = true;
                            setForm((f) => ({ ...f, tasks: u }));
                          }}
                        />
                        <span>Completed</span>
                      ...
                      <label className={styles.checkRow}>
                        <input
                          type="checkbox"
                          checked={
                            form.tasks[stepIndex - 2].isComplete === false
                          }
                          onChange={() => {
                            const u = [...form.tasks];
                            u[stepIndex - 2].isComplete = false;
                            setForm((f) => ({ ...f, tasks: u }));
                          }}
                        />
                  ...
                  <div className={styles.photoSection}>
                    <h4 className={styles.photoSectionTitle}>
                      Before Repair{" "}
                      {currentTask.uploadedBeforeCount ? (
                        <span className={styles.uploadedCount}>
                          ({currentTask.uploadedBeforeCount} uploaded)
                        </span>
                      ) : null}
                    </h4>
                    <input
                    ...
                    />
                    <div className={styles.previewGrid}>
                      {(currentTask.uploadedBeforePreviews || []).map(
                        (src, i) => (
                          <div key={`ub-${i}`} className={styles.previewThumb}>
                            <img
                              src={src}
                              alt=""
                              onClick={() => setEnlargedSrc(src)}
                      ...
                      <div
                        className={styles.uploadBox}
                        onClick={() => photoInputRef.current.before?.click()}
                      >
                        +
                      </div>
                    </div>
                  ...
                  <div className={styles.photoSection}>
                    <h4 className={styles.photoSectionTitle}>
                      After Repair{" "}
                      {currentTask.uploadedAfterCount ? (
                        <span className={styles.uploadedCount}>
                          ({currentTask.uploadedAfterCount} uploaded)
                        </span>
                      ) : null}
                    </h4>
                    <input
                    ...
                    />
                    <div className={styles.previewGrid}>
                      {(currentTask.uploadedAfterPreviews || []).map(
                        (src, i) => (
                          <div key={`ua-${i}`} className={styles.previewThumb}>
                            <img
                              src={src}
                              alt=""
                              onClick={() => setEnlargedSrc(src)}
                      ...
                      <div
                        className={styles.uploadBox}
                        onClick={() => photoInputRef.current.after?.click()}
                      >
                        +
                      </div>
                    </div>
              ...
              <div className={styles.field}>
                ...
                <textarea
                  ...
                  value={form.tasks[stepIndex - 2].notes || ""}
                  onChange={(e) => {
                    const updated = [...form.tasks];
                    updated[stepIndex - 2].notes = e.target.value;
                    setForm((f) => ({ ...f, tasks: updated }));
                  }}
                />
              </div>
          ...
          {step === "final" && (
            <div className={styles.stepBody}>
              ...
              <div className={styles.summaryList}>
                ...
                <div className={styles.summaryCard}>
                  <div className={styles.summaryCardHeader}>
                    <span className={styles.summaryCardName}>
                      Realignment Check
                    </span>
                    <div className={styles.summaryCardActions}>
                      <span
                        className={`${styles.statusBadge} ${realignmentMeta.isSubmitted ? styles.submitted : realignmentMeta.isSaved ? styles.saved : styles.notSaved}`}
                      >
                        {realignmentMeta.isSubmitted
                          ? "SUBMITTED"
                          : realignmentMeta.isSaved
                            ? "SAVED"
                            : "NOT SAVED"}
                      </span>
                      <button
                  ...
                  <div
                    className={`${styles.statusText} ${realignmentChecked ? styles.statusGreen : styles.statusRed}`}
                  >
                  ...
                  {realignmentChecked && (
                    <>
                      <div className={styles.summaryDetail}>
                        ...
                        <span
                          className={
                            needsRepair ? styles.statusRed : styles.statusGreen
                          }
                        >
                          {needsRepair === true
                            ? "Yes"
                ...
                {form.tasks.map((task, idx) => (
                  <div key={idx} className={styles.summaryCard}>
                    <div className={styles.summaryCardHeader}>
                      <div>
                        <div className={styles.summaryCardNum}>
                          Task {idx + 1}
                        </div>
                        <div className={styles.summaryCardName}>
                          {task.name.startsWith("Other")
                            ? task.customName || task.name
                            : task.name}
                        </div>
                      </div>
                      <div className={styles.summaryCardActions}>
                        <span
                          className={`${styles.statusBadge} ${task.isSubmitted ? styles.submitted : task.isSaved ? styles.saved : styles.notSaved}`}
                        >
                          {task.isSubmitted
                            ? "SUBMITTED"
                            : task.isSaved
                    ...
                    <div
                      className={`${styles.statusText} ${task.checked ? styles.statusGreen : styles.statusRed}`}
                    >
                      {task.checked ? "✅ Checked" : "❌ Not Checked"}
                    </div>
                    <div className={styles.photoRow}>
                      {task.uploadedBeforePreviews?.map((src, i) => (
                        <img
                          key={i}
                          src={src}
                          className={styles.thumbSmall}
                    ...
                    {task.notes && (
                      <div className={styles.summaryNotes}>{task.notes}</div>
                    )}
                  </div>
                ))}
              </div>
              ...
              <div className={styles.field}>
                <label className={styles.label}>
                  Completion Date <span className={styles.required}>*</span>
                </label>
                <input
                  type="date"
                  className={styles.input}
                  value={
                    form.completionDate ? form.completionDate.slice(0, 10) : ""
                  }
                  onChange={(e) =>
                    setForm((prev) => ({
                      ...prev,
